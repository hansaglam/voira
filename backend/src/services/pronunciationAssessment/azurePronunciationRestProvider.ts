...
import {
  assessAzurePronunciationRest,
} from './azurePronunciationRestService.js';
import { isAzurePronunciationAssessmentConfigured } from '../azurePronunciationAssessmentService.js';

function mapWords(
  words: Awaited<ReturnType<typeof assessAzurePronunciationRest>>['words'],
): PronunciationWordScore[] {
...
export const azurePronunciationRestProvider: PronunciationAssessmentProvider = {
  async assess(request: PronunciationAssessmentRequest): Promise<PronunciationAssessmentResult> {
    ...
    const azureResult = await assessAzurePronunciationRest({
