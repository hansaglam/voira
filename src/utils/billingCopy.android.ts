...
export function getManageSubscriptionText(): string {
  return 'Aboneliğini Google Play hesabından yönetebilirsin.';
}

export function getManageSubscriptionCancelText(): string {
  return 'Aboneliğini Google Play hesap ayarlarından yönetebilir ya da iptal edebilirsin.';
}

export function getPremiumCancelNote(): string {
  return 'Aboneliğini Google Play hesap ayarlarından yönetebilir ya da iptal edebilirsin. Abonelik otomatik yenilenir.';
}

export function getAboutSpeakPlusBody(): string {
  return 'SpeakPlus, uygulama içi abonelik ile sunulan premium ders paketleri ve gelişmiş geri bildirimlerdir. Abonelik ödemeleri Google Play hesabın üzerinden yönetilir. Satın alımları Profil veya paywall ekranından geri yükleyebilirsin.';
}

export function getTermsSpeakPlusBody(): string {
  return 'SpeakPlus abonelikleri Google Play üzerinden faturalandırılır. Fiyat ve yenileme koşulları satın almadan önce gösterilir. İptal ve iadeler Google Play politikalarına tabidir. Uygulamayı silmek veya veri silme talebi aboneliği iptal etmez; aboneliğini Google Play hesap ayarlarından yönetebilir ya da iptal edebilirsin.';
}
...
export function getPrivacyThirdPartyBody(): string {
  return 'Supabase (kimlik doğrulama / veri), RevenueCat (abonelik entitlement), Google Play (ödemeler), Microsoft Azure Speech (konuşma tanıma ve telaffuz değerlendirmesi), OpenAI (backend etkinse kayıtlı ses üzerinde speech-to-text/transcription ve AI koç geri bildirimi / açıklamalar / öğrenme rehberliği), Render (backend barındırma). Bu hizmetler yalnızca uygulama işlevi için gerekli ölçüde veri işler.';
}

export function getPrivacyPaymentsBody(): string {
  return 'Abonelik ödemeleri Google Play hesabın üzerinden yönetilir. Voira kredi/banka kartı numaralarını almaz veya saklamaz. Aboneliğini Google Play hesap ayarlarından yönetebilir ya da iptal edebilirsin. SpeakPlus erişimi RevenueCat ve Google Play entitlement durumuna göre yönetilir.';
}
...
export function getDataDeletionLocalResetMessage(): string {
  return 'Bu işlem yalnızca bu cihazdaki pratik geçmişini, skorları ve günlük oturum kayıtlarını siler. Hesap silme talebi değildir. Profil tercihlerin korunur. Aktif bir Google Play aboneliğini iptal etmez.';
}

export function getDataDeletionSubscriptionNote(): string {
  return 'Uygulama hesabını veya uygulama verilerini silmek, aktif SpeakPlus aboneliğini iptal etmez. Faturalandırmayı durdurmak için aboneliğini Google Play hesap ayarlarından iptal et.';
}

export function getDataDeletionSpeakPlusNote(): string {
  return 'Uygulama hesabını veya uygulama verilerini silmek, aktif SpeakPlus aboneliğini iptal etmez. SpeakPlus’ı durdurmak için aboneliğini Google Play hesap ayarlarından iptal et.';
}

export function getDataDeletionMayRemainBody(): string {
  return 'Google Play / RevenueCat işlem veya entitlement kayıtları, kimliği belli olmayan günlükler ve yalnızca cihazında kalan veriler (uygulama verisi temizlenene veya uygulama kaldırılana kadar).';
}
...
export function getAccountDeletionSubscriptionWarning(): string {
  return 'Hesabını silmek Google Play aboneliğini otomatik iptal etmez. Aktif aboneliğini Google Play hesap ayarlarından yönetebilirsin.';
}
